...
export default function ForgotPasswordPage() {
  ...
  const handleSendOtp = async (data: { email: string }) => {
    ...
    try {
      ...
      const msg = (err as { response?: { data?: { error?: { message?: string } } } })?.response?.data?.error?.message || 'Failed to send reset code.';
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      const msg = (err as { response?: { data?: { error?: { message?: string } } } })?.response?.data?.error?.message || 'Failed to resend code.';
      setError(msg);
    }
  };
  ...
  const stepDots = [1, 2, 3].map((s) => (
    <div
      key={s}
      className={`h-1 rounded-full transition-all ${s < step ? 'w-9 bg-primary' : s === step ? 'w-13 bg-primary' : 'w-9 bg-gray-200'}`}
    />
  ));
  ...
  return (
    <div className="flex min-h-screen font-satoshi bg-surface text-text">
      <div className="hidden md:flex flex-col justify-center items-center w-[45%] bg-gradient-to-br from-primary-dark via-primary to-primary-light text-white p-16 relative overflow-hidden">
        ...
        <p className="text-lg opacity-90 max-w-[380px] text-center leading-relaxed z-10">
          Your career journey starts here. We will help you get back into your account safely.
        </p>
      </div>
        ...
        <div className="w-full max-w-[440px]">
              ...
              <form onSubmit={emailForm.handleSubmit(handleSendOtp)}>
                <div className="mb-5">
                  <label className="block font-medium text-sm mb-1.5">
                    {AUTH_STRINGS.EMAIL_LABEL} <span className="text-red-500">{AUTH_STRINGS.REQUIRED}</span>
                  </label>
                  <input
                    type="email"
                    className="w-full px-3.5 py-3 border-[1.5px] border-border rounded-[10px] text-sm font-satoshi focus:outline-none focus:border-primary focus:ring-[3px] focus:ring-primary/10"
                ...
                <button type="submit" className="w-full py-3.5 bg-primary text-white rounded-[10px] text-[0.95rem] font-semibold font-satoshi flex items-center justify-center gap-2 hover:bg-primary-hover transition-all disabled:opacity-60" disabled={loading}>
                  <i className={loading ? 'fas fa-spinner fa-spin' : 'fas fa-paper-plane'} /> {AUTH_STRINGS.FORGOT_BUTTON}
                </button>
              </form>
          ...
          {step === 2 && (
            <>
              ...
              <p className="text-text-muted text-[0.95rem] mb-6">We sent a 6-digit code to <strong>{email}</strong></p>
              <div className="flex gap-2.5 justify-center mb-6">
                {otp.map((digit, index) => (
                  <input
                    ...
                    inputMode="numeric"
                    maxLength={1}
                    className={`w-[52px] h-[58px] border-[1.5px] rounded-xl text-center text-2xl font-bold font-satoshi focus:outline-none focus:border-primary focus:ring-[3px] focus:ring-primary/10 ${digit ? 'border-primary bg-primary/[0.04]' : 'border-border'}`}
                    value={digit}
              ...
              <div className="text-center text-sm text-text-muted mb-6">
                ...
                {canResend ? (
                  <button type="button" className="text-primary font-semibold" onClick={handleResend}>Resend code</button>
                ) : (
                  <span className="text-text-light">Resend in <strong>{countdown}</strong>s</span>
                )}
              </div>
              <button className="w-full py-3.5 bg-primary text-white rounded-[10px] text-[0.95rem] font-semibold flex items-center justify-center gap-2 hover:bg-primary-hover transition-all disabled:opacity-60" onClick={handleVerifyOtp} disabled={loading}>
                <i className={loading ? 'fas fa-spinner fa-spin' : 'fas fa-check-circle'} /> Verify Code
              </button>
          ...
          {step === 3 && (
            <>
              ...
              <p className="text-text-muted text-[0.95rem] mb-8">Your identity has been verified. Create a new password.</p>
              <form onSubmit={pwdForm.handleSubmit(handleResetPassword)} className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-3.5 py-3 pr-[42px] border-[1.5px] border-border rounded-[10px] text-sm font-satoshi focus:outline-none focus:border-primary focus:ring-[3px] focus:ring-primary/10"
                ...
                <button type="submit" className="w-full py-3.5 bg-primary text-white rounded-[10px] text-[0.95rem] font-semibold flex items-center justify-center gap-2 hover:bg-primary-hover transition-all disabled:opacity-60" disabled={loading}>
                  <i className={loading ? 'fas fa-spinner fa-spin' : 'fas fa-save'} /> Reset Password
                </button>
              </form>
            </>
          )}
